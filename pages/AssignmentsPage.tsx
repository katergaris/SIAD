...
const AssignmentsPage: React.FC = () => {
  ...
  const [filters, setFilters] = useState({ employeeId: '', courseId: '', status: '' });

  const handleOpenModal = (assignmentToEdit?: TrainingAssignment) => {
    if (!currentSede || !currentYear || !isDataEffectivelyLoaded) {
        alert("Seleziona sede/anno e carica i dati CSV da Admin prima di gestire le assegnazioni.");
        return;
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentAssignment.employeeId || !currentAssignment.courseId || !currentAssignment.assignmentDate || !currentAssignment.completionStatus) {
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-lg shadow-md">
        <div className="flex justify-between items-center mb-1">
          <h1 className="text-3xl font-bold text-gray-800">Assegnazioni Corsi - {currentSede} / {currentYear}</h1>
          <button 
        ...
        <p className="text-xs text-gray-500 mt-1">
            Modifiche attiveranno download di <code>{currentSede}_{currentYear}_assegnazioni.csv</code>. Salvalo manualmente.
        </p>
        {(employees.length === 0 || courses.length === 0) && isDataEffectivelyLoaded && (
          <p className="text-sm text-yellow-700 bg-yellow-50 p-3 rounded-md mt-3">
            Per creare assegnazioni, assicurati di aver caricato CSV con dipendenti e corsi per {currentSede}/{currentYear}.
          </p>
        )}
      </div>
      ...
      {assignments.length === 0 && isDataEffectivelyLoaded && (
          <p className="text-center text-gray-500 text-lg py-8">Nessuna assegnazione creata per {currentSede}/{currentYear}.</p>
      )}
              ...
              {filteredAssignments.map(assign => {
                ...
                return (
                  <tr key={assign.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{employee?.name || assign.employeeId}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{course?.name || assign.courseId}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={currentAssignment.isEditing ? "Modifica Assegnazione" : "Nuova Assegnazione"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="employeeId" className="block text-sm font-medium text-gray-700">Dipendente</label>
            <select name="employeeId" id="employeeId" value={currentAssignment.employeeId || ''} onChange={handleInputChange} required className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm bg-white focus:ring-primary focus:border-primary" disabled={currentAssignment.isEditing || employees.length === 0}>
              <option value="" disabled>{employees.length === 0 ? "Nessun dipendente" : "Seleziona Dipendente"}</option>
              {employees.map(emp => <option key={emp.id} value={emp.id}>{emp.name}</option>)}
            </select>
          </div>
          <div>
            <label htmlFor="courseId" className="block text-sm font-medium text-gray-700">Corso</label>
            <select name="courseId" id="courseId" value={currentAssignment.courseId || ''} onChange={handleInputChange} required className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm bg-white focus:ring-primary focus:border-primary" disabled={currentAssignment.isEditing || courses.length === 0}>
          ...
          <div>
            ...
            <input type="date" name="assignmentDate" id="assignmentDate" value={currentAssignment.assignmentDate || ''} onChange={handleInputChange} required className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary" disabled={currentAssignment.isEditing} />
          </div>
          <div>
            <label htmlFor="completionStatus" className="block text-sm font-medium text-gray-700">Stato Completamento</label>
            <select name="completionStatus" id="completionStatus" value={currentAssignment.completionStatus || ''} onChange={handleInputChange} required className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm bg-white focus:ring-primary focus:border-primary">
              {Object.values(AssignmentStatus).map(status => <option key={status} value={status}>{status}</option>)}
            </select>
          </div>
          {currentAssignment.completionStatus === AssignmentStatus.COMPLETED && (
            <>
              <div>
                ...
                <input type="date" name="completionDate" id="completionDate" value={currentAssignment.completionDate || ''} onChange={handleInputChange} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary" />
              </div>
              <div>
                <label htmlFor="score" className="block text-sm font-medium text-gray-700">Punteggio (opzionale)</label>
                <input type="number" name="score" id="score" value={currentAssignment.score === null || currentAssignment.score === undefined ? '' : currentAssignment.score} onChange={handleInputChange} min="0" max="100" className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary" />
              </div>
            </>
          )}
